...
function generateRequestId(prefix: string) {
  // 送信直前に生成、1送信1ID
  // crypto.randomUUID が使えない環境向けにフォールバックあり
  const uuid =
...
export default function KensetsuCareerSupportGeneralLp() {
  ...
  const landingPath = useMemo(() => {
    const path = router.asPath || "/kensetsu-career-support";
    // クエリは含めても良いが、見通しのために除外したい場合はここで加工
    return path;
  }, [router.asPath]);
  ...
  const validate = () => {
    ...
    const qualifications = splitToArrayByCommaOrNewline(qualificationsText);
    if (qualifications.length === 0) return "保有資格を入力してください";
    return "";
  };
  ...
  const onSubmit = async () => {
    ...
    const payload: Record<string, any> = {
      ...
      // hidden（一般のみ）
      qualifications_text: qualificationsText,
